  ...
  - Tabs
    ...
    - 'Text' tab, displays a 'text' document.
      ...
      - Info sidebar with a grid of key/value items
        ...
        - [ ] Selection information. ❌ The current Vizia textbox integration does not expose selection metadata to the sidebar.
    - 'Image' tab, displays an 'image' document.
      ...
      - [x] Do not allow image to be scaled down.
      - [x] If too big to fit in the window, scrollbars must be present to allow panning
      - [x] Content must be loaded in a thread or async, in the background.
      - Tab State
...
- Bonus points
  - [ ] Native look and feel controls. ❌ The current Vizia UI uses custom-styled widgets rather than native platform controls.
  - [x] Some way of closing all the tabs in one go (e.g. "Close all" button on toolbar).
  - [x] Add 'Display in window' on tab context menu which when clicked displays the document in a new window with no tab bar, and where the window title is the name of the file.
  - [x] Multiple tabs for the same document, e.g. right-click a tab, click 'Duplicate'.  Changes in one tab are reflected in the other.
  - [x] Status bar, showing some active-tab-specific state, e.g. last click location on image tabs. changes when changing tabs. (e.g. IDEs often show line number, offset, and selected line/character counts in the status bar).
  - [x] When two tabs are open, where the file name names of the document are the same, but the directory the file is in is different, show enough of the path to be able to distinguish the two tabs.
  ...
  - [x] Tests for individual components.
  - [x] Integration/Behavioral tests. ⚠️ Current coverage is limited to worker/storage behavior tests; there is no end-to-end UI integration suite yet.